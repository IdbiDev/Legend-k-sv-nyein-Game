    ...
    def place_mobs(self, amount: int):
        ...
        while already_placed < amount:
            ...
            if self.grid[y][x] == 0:
                ...
                if mob == Zombie:
                    self.grid[y][x] = Zombie(loc, randint(200, 300), randint(7, 15), randint(0, 5) / 100, randint(5, 10) / 100, randint(15, 25) / 100)
                elif mob == Goblin:
                    self.grid[y][x] = Goblin(loc, randint(75, 200), randint(3, 8), randint(5, 10) / 100, randint(20, 40) / 100, randint(45, 65) / 100)
                elif mob == Dragon:
                    self.grid[y][x] = Dragon(loc, randint(250, 370), randint(15, 22), randint(0, 5) / 100, randint(25, 45) / 100, randint(25, 45) / 100)
                elif mob == Ghost:
                    self.grid[y][x] = Ghost(loc, randint(50, 100), randint(50, 75), randint(0, 1) / 100, randint(50, 70) / 100, randint(12, 30) / 100)
                elif mob == Gorgon:
                    self.grid[y][x] = Gorgon(loc, randint(150, 200), randint(2, 9), randint(65, 100) / 100, randint(0, 5) / 100, randint(70, 80) / 100)
                    ...
                    self.grid[y][x] = Giant(loc, randint(450, 650), randint(12, 23), randint(0, 20) / 100, randint(0, 2) / 100, randint(50, 80) / 100)
                elif mob == Elf:  # health                dmg                crit                 dodge                         hit
                    self.grid[y][x] = Elf(loc, randint(250, 400), randint(22, 33), randint(10, 23) / 100, randint(14, 30) / 100, randint(30, 43) / 100)
                elif mob == Golem:  # health                  dmg                   crit                    dodge                         hit
                    self.grid[y][x] = Golem(loc, randint(750, 1000), randint(5, 10), randint(7, 14) / 100, randint(0, 10) / 100, randint(7, 20) / 100)
                elif mob == Wolf:  # health                  dmg                   crit                    dodge                         hit
                    self.grid[y][x] = Wolf(loc, randint(150, 225), randint(15, 23), randint(30, 47) / 100, randint(15, 30) / 100, randint(15, 30) / 100)
                elif mob == Snake:  # health                  dmg                   crit                    dodge                         hit
                    self.grid[y][x] = Snake(loc, randint(75, 300), randint(5, 17), randint(27, 38) / 100, randint(5, 15) / 100, randint(20, 33) / 100)
                already_placed += 1
