...
class Player:
    ...
    def to_dict(self):
        return {"name": self.name, "position": self.position, "current_map": self.current_map.id,
                "balance": self.balance, "inventory": self.inventory.to_dict(), "armor_slot": self.armor_slot.to_dict()}

    def get_calculated_stats(self) -> dict[str, float]:
        data = {"base_damage": self.base_damage, "crit_chance": self.crit_chance, "dodge_chance": self.dodge_chance,
                "hit_chance": self.hit_chance, "health": self.health}
