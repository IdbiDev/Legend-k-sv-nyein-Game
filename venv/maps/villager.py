...
def get_type_by_name(name):
    if name in MessageType.BLACKSMITH_NAMES.value.messages:
        return "Blacksmith"
    elif name in MessageType.ARMORER_NAMES.value.messages:
        return "Armorer"
    elif name in MessageType.FARMER_NAMES.value.messages:
        return "Farmer"
    elif name in MessageType.HUNTER_NAMES.value.messages:
        return "Hunter"
    elif name in MessageType.WIZARD_NAMES.value.messages:
        return "Wizard"
...
class Villager:
    ...
    def to_dict(self):
        return {"name": self.name, "location": self.location.position}
...
class Blacksmith(Villager):
    ...
    def interact(self, player):
        ...
        if len(self.current_offers) == 0:
            MessageType.VILLAGER_OUT_OF_STOCK.value.get_dialog_replaced_message(npc=self.name).print()
            clear()
            return
                ...
                match keyboard.read_key(suppress=True):
                    ...
                    case "enter":
                        ...
                        if player.balance - self.current_offers[selected_item] < 0:
                            Dialog(MessageType.SHOP_NOT_ENOUGH_MONEY.value.get_replaced_message(player=player.name,
                                                                                                npc=self.name,
                                                                                                item=selected_item.name)).print()
                        else:
                            player.inventory.add_item(selected_item)
                            player.balance -= self.current_offers[selected_item]
                            Dialog(MessageType.SHOP_BOUGHT.value.get_replaced_message(player=player.name, npc=self.name,
                                                                                      item=selected_item.name)).print()
...
class Armorer(Villager):
    ...
    def refresh_offer(self, player):
        new_offers_selected = []
        new_offers = {}
        offers = [o for o in list(self.offers.keys()) if
                  not player.armor_slot == o and o not in player.inventory.items and o not in player.inventory.talismans]
        for i in range(len(offers)):
            c = random.choice(offers)
